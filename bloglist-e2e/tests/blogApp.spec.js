...
describe('Blog app', () => {
  ...
  test('Login form is shown', async ({ page }) => {
    const loginHeader = await page.locator('h2', { hasText: 'Log in to application' });
  ...
  describe('When logged in', () => {
    ...
    test('a new blog can be created', async ({ page }) => {
      ...
      await expect(blogSummary.getByRole('button', { name: 'View' })).toBeVisible();
    });
    ...
    test('a user cannot see the delete button for a blog they did not create', async ({ page, request }) => {
